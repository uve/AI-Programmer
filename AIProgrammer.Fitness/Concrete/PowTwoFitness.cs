...
{
    /// <summary>
    /// Calculates Math.Pow(2, input). Input and output is a byte value (not ASCII character).
    /// </summary>
    public class PowTwoFitness : FitnessBase
    {
        ...
        /// To use, set _appendCode = PowTwoFitness.Functions in main program.
        /// </summary>
        public static string Functions = ",>,-[-<+>]<+$@++[+[>,$[-<-a!a!$@";

        public PowTwoFitness(GA ga, int maxIterationCount, int maxTrainingCount = 3, string appendFunctions = null)
        ...
        protected override double GetFitnessMethod(string program)
        {
            ...
            for (int i = 0; i < _trainingCount; i++)
            {
                ...
                if (_console.Length > 0)
                {
                    ...
                    Fitness += 256 - Math.Abs(result - Math.Pow(2, input1));
                }
