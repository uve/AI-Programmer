...
namespace AIProgrammer
{
    ...
    class Program
    {
        ...
        private static int _genomeSize = 1024; // Number of programming instructions in generated program (size of genome array). loops).
        private static int _maxGenomeSize = 2048; // The max length a genome may grow to (only applicable if _expandAmount > 0).
        private static int _maxIterationCount = 5000; // Max iterations a program may run before being killed (prevents infinite loops).
        private static int _expandAmount = 1024; // The max genome size will expand by this amount, every _expandRate iterations (may help learning). Set to 0 to disable.
