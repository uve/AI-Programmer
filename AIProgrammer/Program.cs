...
namespace AIProgrammer
{
    ...
    class Program
    {
        ...
        private static string _appendCode = SimpleNumbers.Functions; // Program code, containing functions, that will be appended to main program code
        ...
        private static int _genomeSize = 256; // Number of programming instructions in generated program (size of genome array). loops).
        private static int _maxGenomeSize = 256; // The max length a genome may grow to (only applicable if _expandAmount > 0).
        private static int _maxIterationCount = 5000; // Max iterations a program may run before being killed (prevents infinite loops).
        private static int _expandAmount = 128; // The max genome size will expand by this amount, every _expandRate iterations (may help learning). Set to 0 to disable.
        ...
        private static IFitness GetFitnessMethod()
        {
            ...
            //return new SimpleNumbers(_ga, _maxIterationCount, 6, SimpleNumbers.Functions);
            //return new PowTwoFitness(_ga, _maxIterationCount, 4, PowTwoFitness.Functions);
            return  new AddFitness(_ga, _maxIterationCount, 4);
        }
